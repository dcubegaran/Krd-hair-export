...
import { QuoteRequest } from "@/lib/types/database"
...
export default function AdminPanel() {
  ...
  const [products, setProducts] = useState([
    { id: 1, name: "Virgin Brazilian Straight", sku: "VBS-001", stock: 45, price: 120, status: "Active" },
    { id: 2, name: "Peruvian Body Wave", sku: "PBW-002", stock: 23, price: 135, status: "Active" },
    { id: 3, name: "Indian Curly Closure", sku: "ICC-003", stock: 12, price: 85, status: "Low Stock" },
  ])
  ...
  const handleProductAdded = (newProduct: any) => {
    ...
    setProducts(prev => [...prev, {
      ...
      price: newProduct.pricePerBundle,
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      setLoading(true)
      try {
        // Load quotes statistics
        const quoteStats = await QuoteService.getQuoteStats()
        setStats(prev => ({
          ...prev,
          totalQuotes: quoteStats.totalQuotes,
          submittedQuotes: quoteStats.submittedQuotes,
          quotedQuotes: quoteStats.quotedQuotes,
  ...
  const recentOrders = [
    { id: "ORD-001", customer: "Sarah Johnson", amount: 1250, status: "Processing", date: "2024-01-15" },
    { id: "ORD-002", customer: "Mike Chen", amount: 890, status: "Shipped", date: "2024-01-14" },
    { id: "ORD-003", customer: "Emma Davis", amount: 2100, status: "Delivered", date: "2024-01-13" },
  ]
        ...
        <header className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="flex h-16 items-center justify-between px-6">
            <div className="flex items-center space-x-4">
              <h1 className="text-2xl font-bold text-primary">KRD Admin Panel</h1>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm text-muted-foreground">Welcome, {user?.displayName || 'Admin'}</span>
          ...
          <main className="flex-1 p-6">
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-muted-foreground">Manage your hair product inventory</p>
                  </div>
            ...
            {activeTab === "customers" && (
              <div className="space-y-6">
                ...
                <Card>
                  <CardContent className="p-6">
                    <p className="text-center text-muted-foreground">Customer management interface coming soon...</p>
                  </CardContent>
                </Card>
              </div>
            )}
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-muted-foreground">Manage customer quote requests and responses</p>
                  </div>
                  <Badge variant="secondary">{quotes.length} Total Quotes</Badge>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">${stats.totalQuotes > 0 ? (quotes.reduce((sum, quote) => sum + (quote.totalValue || 0), 0) / stats.totalQuotes).toFixed(0) : '0'}</div>
                      <p className="text-xs text-muted-foreground">Average per quote</p>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Latest customer quote submissions</CardDescription>
                  </CardHeader>
                  <CardContent>
                    {loading ? (
                          ...
                          <div key={quote.id} className="flex items-center justify-between p-4 border rounded-lg">
                            <div className="space-y-1">
                              <div className="flex items-center gap-2">
                                ...
                                <Badge variant={quote.status === 'submitted' ? 'default' : quote.status === 'quoted' ? 'secondary' : 'outline'}>
                                  {quote.status}
                                </Badge>
                              </div>
                            ...
                            <div className="text-right space-y-1">
                              <p className="font-medium">${quote.totalValue?.toLocaleString()} {quote.currency}</p>
                              <p className="text-sm text-muted-foreground">
                                {quote.createdAt ? new Date(quote.createdAt as any).toLocaleDateString() : 'N/A'}
                              </p>
                              <div className="flex items-center gap-2">
                                <Button variant="ghost" size="sm">
                                  <Eye className="h-4 w-4" />
